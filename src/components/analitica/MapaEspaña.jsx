...
export default function MapaEspaña() {
    ...
    const fetchProvinceData = async () => {
      try {
        ...
        const [currentResponse, previousResponse] = await Promise.all([
          fetch(`${baseUrl}/api/clients/mapa/resumen-provincias?anio=${anio}`, { headers }),
          fetch(`${baseUrl}/api/clients/mapa/resumen-provincias?anio=${previousYear}`, { headers }),
        ]);

        if (!currentResponse.ok) throw new Error(`Error ${currentResponse.status} al cargar provincias`);
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-6">
      <div className="mb-6 overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 p-6 text-white shadow-xl">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-blue-200">
              Analítica geográfica
            </p>
            <h1 className="mt-2 text-3xl font-bold">Mapa de España</h1>
            <p className="mt-2 max-w-3xl text-sm text-blue-100">
              Visualiza facturación, clientes, ticket medio y crecimiento por provincia. La comparación se realiza contra el año anterior para detectar zonas fuertes, riesgos y oportunidades.
            </p>
          </div>

          <div className="flex flex-wrap items-center gap-3">
            <label className="text-sm font-medium text-blue-100" htmlFor="anio-mapa-espana">
              Año
            </label>
            <select
              id="anio-mapa-espana"
